...
TbMap.prototype.updateFilters = function(categoriesCount) {
	const $globalSourceFilter = $(`.filter-${source}-all`);
	let cantFilter,
		$filter,
		sourceGlobalCount = 0;
	...
	$globalSourceFilter.prop('disabled', 0 >= sourceGlobalCount);
	$globalSourceFilter.closest('.source-filter').toggleClass('disabled', 0 >= sourceGlobalCount);
	$globalSourceFilter.next('span').append(` (${sourceGlobalCount})`);
};
